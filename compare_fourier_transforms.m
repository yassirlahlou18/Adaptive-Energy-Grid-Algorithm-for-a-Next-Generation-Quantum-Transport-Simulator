function compare_fourier_transforms()
    ...
    % nufft on the adaptive grid
    [k_adapt, F_adapt] = perform_nufft(column_adapt, E_adapt, length(column_adapt));
    ...
    column_GL_fine = GL_fine(:, 1);
    column_GG_fine = GG_fine(:, 1);
    ...
    column_GL_adapt = GL_adapt(1,:);
    column_GG_adapt = GG_adapt(1,:);
    
     % Perform NUFFT on the fine grid
    [k_fine, F_GL_fine] = perform_nufft2(GL_fine, E_fine, length(column_GL_fine));
    [~, F_GG_fine] = perform_nufft2(GG_fine, E_fine, length(column_GG_fine));
    
    % Perform NUFFT on the adaptive grid
    [k_adapt, F_GL_adapt] = perform_nufft2(GL_adapt, E_adapt2, length(column_GL_adapt));
    [~, F_GG_adapt] = perform_nufft2(GG_adapt, E_adapt2, length(column_GG_adapt));
