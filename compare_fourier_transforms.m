function compare_fourier_transforms()
    ...
    mat_fine = struct('Nx', Nx_fine, 'dE', dE*ones(1, Nx_fine-1), 'V', V0*ones(1, Nx_fine),'Temp', Temp,'x', x_fine, 'm', m*ones(1, Nx_fine), 'rgf', 1);
    ...
    mat_adapt = struct('Nx', Nx_adapt, 'dE', dE_adapt, 'V', V0*ones(1, Nx_adapt), 'Temp', Temp,'x', subdivisions, 'm', m*ones(1, Nx_adapt), 'rgf', 1);
    [E_adapt, ~, ~, ~, GR_adapt] = solve_transport_negf({{mat_adapt}});
    
    column_adapt = diag(GR_adapt);
    fprintf('Dimensions of column: %d x %d\n', size(GR_adapt, 1), size(GR_adapt, 2));

    % fft on the fine grid
  % nufft on the adaptive grid
    [k_fine, F_fine] = perform_nufft(column_fine, x_fine, length(column_fine));
